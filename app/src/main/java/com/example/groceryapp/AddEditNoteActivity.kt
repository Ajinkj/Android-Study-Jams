...
class AddEditNoteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        saveBtn.setOnClickListener {
            ...
            val Quantity = QuantityEdt.text.toString()
            val noteDescription = noteEdt.text.toString()
            // on below line we are checking the type
            // and then saving or updating the data.
            if (noteType.equals("Edit")) {
                if (noteTitle.isNotEmpty() && Quantity.isNotEmpty()) {
                    val sdf = SimpleDateFormat("dd MMM, yyyy - HH:mm")
                    val currentDateAndTime: String = sdf.format(Date())
                    val updatedNote = Note(noteTitle,Quantity, noteDescription, currentDateAndTime)
                ...
                if (noteTitle.isNotEmpty() && Quantity.isNotEmpty()) {
                    ...
                    viewModal.addNote(Note(noteTitle,Quantity, noteDescription, currentDateAndTime))
